...
$.fn.fullscreenconainer = function(options) {
    ...
    var func = {
        ...
        resize: function(obj) {
           ...
           //console.log(windowH + ' / ' + $('section.footer').height())
...
$.fn.googleMap = function (options) {
    ...
    var latLng = new google.maps.LatLng(settings.map_lat, settings.map_long);
    ...
    var myLatLng = new google.maps.LatLng(settings.map_lat, settings.map_long);
...
$.fn.googleMap___ = function(options) {
    var el = $(this);
    var settings = $.extend({
        'lat':'37.536131',
        'long':'25.221395',
        'imgpin':'../images/icons/pin.png',
        'iconsize':[85, 98],
        'iconanchor':[38,98],
...
$.fn.mainmenuCss = function(options) {
    ...
    var myScroller = new ScrollToggle($('section .content-wep').eq(0).position().top, function (e) {
        ...
        $('a.upbtn').removeClass('fadeOut').addClass('fadeIn');
    }, function () {
        el.removeClass('mainnavbar-wrp-onscrolldonw');
        $('a.upbtn').removeClass('fadeIn').addClass('fadeOut');
    }); 
...
$.fn.scrolltopos = function(options) {
    ...
    var func = {
        ...
        init: function() {
           ...
           obj.on('click', function(e){
               e.preventDefault();
               $('body').stop(true,true).scrollTo($('section').eq(0).find('div.content-wep'), 600, {axis:'y'});
           });
...
$.fn.maincontent_show_hide = function (options) {
 ...
 $.fn.excol = function (options) {
     var el = $(this);
     var settings = $.extend({
        //'excolBtn': el.find('.excol'),
        'expandBtn': el.find('.expand'),
        'collapseBtn': el.find('.collapse'),
        'container': '.slideFixWidth990room.rtop'
    }, options);
    
    /*$(settings.excolBtn).on("click", function (e) {
        e.preventDefault();
        $(settings.excolBtn).toggleClass('hide-btn');
        $(settings.container).toggleClass('show-cont');
    });*/
     
    var wrapHeight = $(settings.container).height();
    
    if(wrapHeight < 890) {
        $(settings.container).height('auto');
        $(settings.expandBtn).addClass('hide-btn');
        $(settings.collapseBtn).addClass('hide-btn');
    } else {
        $(settings.container).height('890px');
        $(settings.container).css({
            overflow: 'hidden'
}       );
    }
     
    $(settings.expandBtn).on("click", function (e) {
        e.preventDefault();
        $(settings.expandBtn).addClass('hide-btn');
        $(settings.collapseBtn).removeClass('hide-btn');
        $(settings.container).addClass('show-cont');
    });
    
    $(settings.collapseBtn).on("click", function (e) {
        e.preventDefault();
        $(settings.collapseBtn).addClass('hide-btn');
        $(settings.expandBtn).removeClass('hide-btn');
        $(settings.container).removeClass('show-cont');
        
        var offset = -($('.navbar').height()) * 2;
        
        $('body').stop(true, true).scrollTo($(settings.container), 600, {axis: 'y', offset: {top: offset}, onAfter: function () {
        }});
    });
 };