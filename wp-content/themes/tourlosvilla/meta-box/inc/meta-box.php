	...
	class RW_Meta_Box
	{
		...
		function __construct( $meta_box )
		{
			...
			add_action( 'add_attachment', array( $this, 'save_post' ) );
                        ...
                        add_filter( 'cmb_iscontentpage', array( $this, 'add_plaincontenteplate' ), 10, 2 );
                        //add_action( 'add_plaincontenteplate', array( $this, 'add_plaincontenteplate' ) );
		}
		...
		function add_meta_boxes()
		{
			...
			{
                                //if( apply_filters( 'cmb_show_on', true, $this->meta_box ) ) {
                                if( apply_filters( 'cmb_show_on', true, $this->meta_box ) && apply_filters( 'cmb_iscontentpage', true, $this->meta_box ) ) {
                                    add_meta_box(
                                            $this->meta_box['id'],
                                            $this->meta_box['title'],
                                            array( $this, 'show' ),
                ...
                function add_plaincontenteplate( $display, $meta_box ) {
                    
                    if( $meta_box['contentpage'] == true ) {
                ...
                function add_for_page_template( $display, $meta_box ) {
                        ...
                        $ar[] = $meta_box['show_on']['value'];
                        
                        if ( !empty($meta_box['show_on']['multi']) ) {
                            foreach($meta_box['show_on']['multi'] as $v) {
                                $ar[] = $v['value'];
                            }
                        }
